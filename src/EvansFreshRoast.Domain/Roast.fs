module EvansFreshRoast.Domain.Roast
...
let getInvoiceAmt (allCoffees: seq<Coffee>) { LineItems = lineItems } =
    lineItems
    |> Seq.sumBy (fun kvp ->
        let price =
            allCoffees
...
let execute (allCustomers: seq<CustomerProjection>) (allCoffees: seq<Coffee>) (today: LocalDate) roast cmd =
    match cmd with
...
let apply (allCoffees: seq<Coffee>) roast event =
    match event with
...
let getOfferedCoffeeSummary (allCoffees: seq<Coffee>) roast =
    let getSummary (referenceId, (coffee: Coffee)) =
