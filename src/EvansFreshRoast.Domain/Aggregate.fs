module EvansFreshRoast.Domain.Aggregate
...
let createHandler
    ...
    fun id cmd ->
        async {
            ...
            | Error err -> return Error <| FailedToLoadEvents err
            | Ok events ->
                let currentVersion =
                    events
                    |> List.tryLast
                ...
                let execute currentState expectedVersion =
                    async {
                ...
                | [], _ -> return! execute aggregate.Empty AggregateVersion.one
                | _, Some version ->
                    let currentState =
                        events
                ...
                | _ -> return Error FailedToGetCurrentVersion
        }
